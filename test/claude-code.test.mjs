...
function prepared(operation) {
  HOSTS["claude-code"].supported = true;
  try { return operation(); }
  finally { HOSTS["claude-code"].supported = false; }
}
...
test("Claude Code remains publicly gated pending authenticated attestation", () => {
  assert.equal(HOSTS["claude-code"].supported, false);
  assert.throws(() => resolveHost("claude-code"), /not supported yet/);
});
...
test("Claude rendering is gated, inherits by default, and uses approved aliases", () => {
  ...
  try {
    const inherited = enablePrepared({ host: "claude-code", scope: "project", cwd, only: ["builder"], skill: false });
    ...
    enablePrepared({ host: "claude-code", scope: "project", cwd, only: ["builder"], profile: "claude-code", skill: false });
    assert.match(readFileSync(join(cwd, ".claude", "agents", "builder.md"), "utf8"), /^model: sonnet$/m);
    assert.throws(() => enablePrepared({ host: "claude-code", scope: "project", cwd, only: ["builder"], profile: "openai" }), /not valid/);
...
test("Claude discovery uses explicit activation and contains no Pi-only controls", () => {
  ...
  for (const text of [generateSkill({ hostKey: "claude-code", enabled, personas: loadPersonas() }), generateBrief({ hostKey: "claude-code", enabled })]) {
...
test("Claude lifecycle tracks discovery and preserves foreign and modified content", () => {
  ...
  try {
    enablePrepared({ host: "claude-code", scope: "project", cwd, only: ["builder", "pm"], brief: true });
    ...
    disablePrepared({ host: "claude-code", scope: "project", cwd, only: ["builder"], force: false });
    ...
    disablePrepared({ host: "claude-code", scope: "project", cwd, only: ["builder"], force: true });
    ...
    disablePrepared({ host: "claude-code", scope: "project", cwd, only: ["pm"] });
...
test("malformed brief markers only block operations that would mutate the brief", () => {
  ...
  try {
    ...
    enablePrepared({ host: "claude-code", scope: "project", cwd, only: ["builder"], skill: false });
    assert.equal(existsSync(join(cwd, ".claude", "agents", "builder.md")), true);
    assert.throws(() => enablePrepared({ host: "claude-code", scope: "project", cwd, only: ["pm"], brief: true }), /invalid Staffed brief/);
...
test("forced discovery writes replace symlinks and directories without following them", () => {
  ...
  try {
    ...
    enablePrepared({ host: "claude-code", scope: "project", cwd, only: ["builder"], brief: true, force: true });
    ...
    disablePrepared({ host: "claude-code", scope: "project", cwd, force: true });
    mkdirSync(skill, { recursive: true });
    rmSync(brief, { force: true });
    mkdirSync(brief);
    enablePrepared({ host: "claude-code", scope: "project", cwd, only: ["pm"], brief: true, force: true });
...
test("forced discovery replacement refuses a non-empty skill directory before agent mutation", () => {
  ...
  try {
    ...
    assert.throws(() => enablePrepared({ host: "claude-code", scope: "project", cwd, only: ["builder"], force: true }), /non-empty discovery directory/);
...
test("forced discovery replacement refuses a non-empty brief directory before agent mutation", () => {
  ...
  try {
    ...
    assert.throws(() => enablePrepared({ host: "claude-code", scope: "project", cwd, only: ["builder"], skill: false, brief: true, force: true }), /non-empty discovery directory/);
    assert.equal(readFileSync(victim, "utf8"), "preserve me");
    assert.equal(readFileSync(agent, "utf8"), "foreign agent");
  } finally { rmSync(cwd, { recursive: true, force: true }); }
});
...
test("discovery failure occurs before forced agent replacement", () => {
  ...
  try {
    ...
    assert.throws(() => enablePrepared({ host: "claude-code", scope: "project", cwd, only: ["builder"], force: true }), /ENOTDIR/);
    assert.equal(readFileSync(agent, "utf8"), "foreign agent");
  } finally { rmSync(cwd, { recursive: true, force: true }); }
});
...
test("--no-brief skips malformed tracked brief preflight during disable", () => {
  ...
  try {
    enablePrepared({ host: "claude-code", scope: "project", cwd, only: ["builder"], brief: true });
    const brief = join(cwd, "CLAUDE.md"); writeFileSync(brief, "<!-- staffed:start -->\nbroken\n");
    disablePrepared({ host: "claude-code", scope: "project", cwd, only: ["builder"], brief: false });
...
test("malformed manifest records block forced removal", () => {
  ...
  try {
    enablePrepared({ host: "claude-code", scope: "project", cwd, only: ["builder"] });
    ...
    assert.throws(() => disablePrepared({ host: "claude-code", scope: "project", cwd, force: true }), /manifest files.builder.md/);
...
test("malformed discovery records also block forced removal", () => {
  ...
  try {
    ...
    writeFileSync(manifestFile, JSON.stringify(manifest));
    assert.throws(() => disablePrepared({ host: "claude-code", scope: "project", cwd, force: true }), /manifest discovery.skill/);
...
test("nested duplicate names and malformed manifests block without mutation", () => {
  ...
  try {
    ...
    assert.throws(() => enablePrepared({ host: "claude-code", scope: "project", cwd, only: ["builder"], force: true }), /collision/);
    assert.equal(existsSync(join(cwd, ".claude", "agents", "builder.md")), false);
    ...
    assert.throws(() => enablePrepared({ host: "claude-code", scope: "project", cwd, only: ["builder"] }), /invalid Staffed manifest/);
    assert.equal(statusPrepared({ host: "claude-code", scope: "project", cwd }).manifestError.includes("JSON"), true);
  } finally { rmSync(cwd, { recursive: true, force: true }); }
});
